...
pub fn parse_file_test() {
  let assert Ok(docs) = yay.parse_file("./test/multi_document.yaml")
...
pub fn selector_test() {
  let assert Ok([doc]) = yay.parse_file("./test/test.yaml")
...
pub fn unicode_test() {
  let assert Ok([doc]) = yay.parse_file("./test/unicode_test.yaml")
...
pub fn error_test() {
  let node =
    yay.NodeSeq([yay.NodeMap([#(yay.NodeStr("valid"), yay.NodeNil)])])
...
pub fn duplicate_key_test() {
  let assert Ok(docs) = yay.parse_file("./test/duplicate_keys.yaml")
